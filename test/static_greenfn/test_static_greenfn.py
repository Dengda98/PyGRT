...
pymod.compute_static_grn(depsrc=depsrc, deprcv=deprcv, dists=dists)
pymod.compute_static_grn(depsrc=depsrc, deprcv=deprcv, dists=dists, calc_upar=True)
pymod.compute_static_grn(depsrc=depsrc, deprcv=deprcv, dists=dists, Length=20)

pymod.compute_static_grn(depsrc=depsrc, deprcv=deprcv, dists=dists, Length=20, converg_method="DCM")
pymod.compute_static_grn(depsrc=depsrc, deprcv=deprcv, dists=dists, Length=20, converg_method="PTAM")
pymod.compute_static_grn(depsrc=depsrc, deprcv=deprcv, dists=dists, Length=20, converg_method="none")

pymod.compute_static_grn(depsrc=depsrc, deprcv=deprcv, dists=dists, k0=4, keps=1e-3)
pymod.compute_static_grn(depsrc=depsrc, deprcv=deprcv, dists=dists, k0=4, stats=True)
...
pymod_m.compute_static_grn(depsrc=depsrcs, deprcv=deprcvs, dists=dists)
...
pymod_ms.compute_static_grn(depsrc=[1.0, 2.0], deprcv=0.0, dists=dists, calc_upar=True)

# 仅多台站深度
pymod_mr = pygrt.PyModel1D(stgrn="stgrn_py_mr.nc", modelpath=modname)
pymod_mr.compute_static_grn(depsrc=2.0, deprcv=[0.0, 0.5], dists=dists)
...
# -------------------- 错误 / 警告（Python）--------------------
try:
    pymod_m.compute_static_grn(depsrc=-1.0, deprcv=0.0, dists=dists)
    raise AssertionError("negative depsrc should raise")
except ValueError:
    pass

try:
    pymod_m.compute_static_grn(depsrc=1.0, deprcv=-0.5, dists=dists)
    raise AssertionError("negative deprcv should raise")
except ValueError:
    pass

try:
    pymod_m.compute_static_grn(depsrc=[], deprcv=0.0, dists=dists)
...
with warnings.catch_warnings(record=True) as w:
    ...
    pymod_m.compute_static_grn(depsrc=depsrcs, deprcv=deprcvs, dists=dists, stats=True)
