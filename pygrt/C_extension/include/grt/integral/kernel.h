...
typedef void (*GRT_KernelFunc) (
    MODEL1D *mod1d, const real_t k, cplxChnlGrid QWV,
...
void grt_GRT_matrix(MODEL1D *mod1d, const real_t k);

/** 从广义 R/T 矩阵出发，计算每个震源对应的核函数 QWV。 作为 kernel 函数中的第二部分 */
void grt_GRT_build_QWV(
    MODEL1D *mod1d, cplxChnlGrid QWV,
    bool calc_uiz, cplxChnlGrid QWV_uiz);
...
/** 静态广义反射透射系数矩阵 */
void grt_static_GRT_matrix(MODEL1D *mod1d, const real_t k);
