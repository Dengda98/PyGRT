...
void source_coef(
    ...
    MYCOMPLEX coef[GRT_SRC_M_COUNTS][GRT_SRC_QWV_COUNTS][2])
{
    // 先全部赋0 
    ...
    coef[0][0][0] = tmp = src_kaka * src_a_inv;         coef[0][0][1] = tmp;    
    
    // 垂直力源 (4.6.15)
    coef[1][0][0] = tmp = -RONE;                         coef[1][0][1] = - tmp;
    coef[1][1][0] = tmp = -k * src_b_inv;                coef[1][1][1] = tmp;

    // 水平力源 (4.6.21,26), 这里可以把x1,x2方向的力转到r,theta方向
    // 推导可发现，r方向的力形成P,SV波, theta方向的力形成SH波
    // 方向性因子包含水平力方向与震源台站连线方向的夹角
    coef[2][0][0] = tmp = -k * src_a_inv;                  coef[2][0][1] = tmp;
    coef[2][1][0] = tmp = -RONE;                           coef[2][1][1] = - tmp;
    coef[2][2][0] = tmp = src_kbkb * k_inv * src_b_inv;    coef[2][2][1] = tmp;

    // 剪切位错 (4.8.34)
    // m=0
    coef[3][0][0] = tmp = (RTWO*src_kaka - RTHREE*kk) * src_a_inv;    coef[3][0][1] = tmp;
    coef[3][1][0] = tmp = -RTHREE*k;                                  coef[3][1][1] = - tmp;
    // m=1
    coef[4][0][0] = tmp = RTWO*k;                              coef[4][0][1] = - tmp;
    coef[4][1][0] = tmp = (RTWO*kk - src_kbkb) * src_b_inv;    coef[4][1][1] = tmp;
    coef[4][2][0] = tmp = - src_kbkb * k_inv;                  coef[4][2][1] = - tmp;

    // m=2
    coef[5][0][0] = tmp = - kk * src_a_inv;                    coef[5][0][1] = tmp;
    coef[5][1][0] = tmp = - k;                                 coef[5][1][1] = - tmp;
    coef[5][2][0] = tmp = src_kbkb * src_b_inv;                coef[5][2][1] = tmp;
