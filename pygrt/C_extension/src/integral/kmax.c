    ...
    real_t kmax_init, real_t kmax_low, real_t kmax_ref, size_t *Ncount)
{
    ...
    while(kmax < kmax_ref){
        ...
        GRT_LOOP_ChnlGrid(im, c){
            ...
            F[im][c] = sqrt(kmax) * (fabs(QWV[im][c]) + fabs(QWVz[im][c]));
            Fmax[im][c] = GRT_MAX(F[im][c], Fmax[im][c]);

            if(kmax >= k_low_eff &&
                F[im][c] <= GRT_KMAX_DECAY_ZERO_TOL * (Fmax[im][c] + 1e-30)){
                nval[im][c]++;
            } else {
                nval[im][c] = 0;
            }

            if(nval[im][c] < GRT_KMAX_NVAL_NEED){
                all_converged = false;
            }
        }
    ...
    real_t kmax_init, real_t kmax_low, real_t kmax_ref, size_t *Ncount)
{
    ...
    while(kmax < kmax_ref){
        ...
        GRT_LOOP_ChnlGrid(im, c){
            ...
            F[im][c] = fabs(QWV[im][c]) + fabs(QWVz[im][c]);
            dF[im][c] = fabs(QWV[im][c] - QWV_const[im][c])
                       + fabs(QWVz[im][c] - QWVz_const[im][c]);
            Fmax[im][c] = GRT_MAX(F[im][c], Fmax[im][c]);

            // dF 是当前核函数与前一采样点（收敛值估计）之间的差值
            if(kmax >= k_low_eff &&
                dF[im][c] <= GRT_KMAX_DECAY_CONST_TOL * (Fmax[im][c] + 1e-30)){
                nval[im][c]++;
            } else {
                nval[im][c] = 0;
            }
