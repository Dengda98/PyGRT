...
IntOrSequence = Union[int, Sequence[int], NDArray[np.integer[Any]]]
...
def _format_mode_option(values: IntOrSequence, name: str = "modes") -> str:
    """将阶数选择格式化为 ``-N`` 的值"""
    if isinstance(values, (str, bytes)):
        raise TypeError(f"{name} must be an integer or a 1-D sequence of integers, not a string.")
    try:
        arr = np.asarray(values, dtype=np.float64)
    except (TypeError, ValueError):
        raise TypeError(f"{name} must be an integer or a 1-D sequence of integers.") from None
    if arr.ndim > 1:
        raise ValueError(f"{name} must be an integer or a 1-D sequence of integers.")
    arr = np.atleast_1d(arr)
    ...
    if modes[0] < 0 or (len(modes) > 1 and modes[1] < 0):
        raise ValueError(f"{name} must be nonnegative.")
    if len(modes) > 1 and modes[0] > modes[1]:
        raise ValueError(f"{name} start must not exceed its end.")
    if len(modes) == 3 and modes[2] <= 0:
        raise ValueError(f"{name} interval must be positive.")
...
class PyModel1D:
    ...
    def eigenfn(
        ...
        *,
        ...
        freqs: Optional[FloatOrSequence] = None,
        periods: Optional[FloatOrSequence] = None,
        modes: Optional[IntOrSequence] = None,
        all_modes: bool = False,
        eigenfn_path: Optional[PathLike] = None,
        depths: Optional[FloatOrSequence] = None,
        group_path: Optional[PathLike] = None,
        csens_path: Optional[PathLike] = None,
        usens_path: Optional[PathLike] = None,
        egy_path: Optional[PathLike] = None,
        sensitivity_dz: Optional[float] = None,
        print_log: bool = True,
    ):
        r"""
        Compute modal eigenfunctions and derived quantities with ``grt eigenfn``.

        The model path is read from the NetCDF dispersion file generated by
        :meth:`eigenv`. Set one or more output paths to request eigenfunctions,
        group velocity, energy integrals, or phase/group-velocity sensitivity
        kernels. All arguments must be passed by keyword.

        :param    phase_path:      Input NetCDF phase-velocity dispersion path.
        :param    freqs:           Optional one-, two- or three-value frequency
                                   selection in Hz. Mutually exclusive with
                                   ``periods``.
        :param    periods:         Optional one-, two- or three-value period
                                   selection in s. Mutually exclusive with
                                   ``freqs``.
        :param    modes:           Optional one-, two- or three-value mode
                                   selection ``(n1[, n2[, dn]])``.
        :param    all_modes:       If true, pass bare ``-N``. Mutually exclusive
                                   with ``modes``.
        :param    eigenfn_path:    Output NetCDF eigenfunction path.
        :param    depths:          Optional one- or three-value depth selection
                                   for ``eigenfn_path`` in km.
        :param    group_path:      Output NetCDF group-velocity dispersion path.
        :param    csens_path:      Output phase-velocity sensitivity path.
        :param    usens_path:      Output group-velocity sensitivity path.
        :param    egy_path:        Output energy-integral path.
        :param    sensitivity_dz:  Depth interval in km for sensitivity and
                                   energy-integral calculations.
        :param    print_log:       If false, suppress command output and
                                   capture it on failure.

        :return: ``None``. Requested NetCDF files are written to their output paths.
        """
        if modes is not None and all_modes:
            raise ValueError("modes and all_modes are mutually exclusive.")
        if depths is not None and eigenfn_path is None:
            raise ValueError("depths requires eigenfn_path.")

        command = {
            "subcommand": "eigenfn",
            "C": f"-C{Path(phase_path)}",
        }
        freq_option = _format_freqs_or_periods_option(freqs, periods, allowed_sizes=(1, 2, 3))
        if freq_option is not None:
            command["F"] = f"-F{freq_option}"
        if modes is not None:
            command["N"] = f"-N{_format_mode_option(modes)}"
        elif all_modes:
            command["N"] = "-N"

        if eigenfn_path is not None:
            _ensure_parent(eigenfn_path)
            option = str(Path(eigenfn_path))
            if depths is not None:
                depth_option = _format_slash_option(depths, "depths", (1, 3), kind="depth")
                option += f"+z{depth_option}"
            command["W"] = f"-W{option}"
        if group_path is not None:
            _ensure_parent(group_path)
            command["U"] = f"-U{Path(group_path)}"
        ...
        if sensitivity_options:
            command["K"] = "-K" + "".join(sensitivity_options)
        run_grt(list(command.values()), print_log=print_log)
    ...
    def modsum(
        ...
        *,
        ...
        freqband: Optional[FloatOrSequence] = None,
        modes: Optional[IntOrSequence] = None,
        all_modes: bool = False,
        upsampling_n: int = 1,
        delayT0: Optional[float] = None,
        delayV0: Optional[float] = None,
        ...
        r"""
        Compute surface-wave Green's functions with ``grt modsum``.

        ``phase_path`` must be a dispersion file generated by :meth:`eigenv`
        with an equally spaced frequency grid. Source and receiver depths may
        be scalars or strictly ascending sequences; all depth combinations are
        computed when sequences are supplied. Results are written to
        ``output_path`` or, when it is omitted, to the ``grn`` root configured
        in the constructor. All arguments must be passed by keyword.
        ...
        :param    dists:           Epicentral distance or strictly ascending
                                   distance sequence in km.
        :param    output_path:     Output Green's-function root. If omitted,
                                   use ``self.grn``.
        :param    freqband:        Optional ``(f1, f2)`` frequency range in Hz.
        :param    modes:           Optional one-, two- or three-value mode
                                   selection ``(n1[, n2[, dn]])``.
        :param    all_modes:       If true, pass bare ``-N``. Mutually exclusive
                                   with ``modes``.
        :param    upsampling_n:    Frequency-domain zero-padding factor.
        :param    delayT0:         Time delay at zero distance in s. Supplying this,
                                   ``delayV0`` or ``ref_first_p`` enables ``-E``.
        :param    delayV0:         Reference velocity for the time delay in km/s.
                                   Used only when ``ref_first_p`` is false.
        :param    ref_first_p:     Whether to use the first P-wave arrival as the
                                   reference for the time delay (CLI ``-Ep``).
        :param    gf_source:       Source types to calculate from ``EX``, ``VF``,
        ...
        if output_path is None:
            if self.grn is None:
                raise RuntimeError("Pass output_path= or grn= to PyModel1D(...) before modsum().")
            output = Path(self.grn)
        else:
        ...
        if freqband is not None:
            command["F"] = f"-F{_format_slash_option(freqband, 'freqband', (2,), kind='frequency')}"
        if modes is not None:
            command["N"] = f"-N{_format_mode_option(modes)}"
        elif all_modes:
            command["N"] = "-N"
        ...
        if gf_source is not None:
            ...
            if not codes:
                raise ValueError("gf_source must contain at least one source type.")
