...
在 |yao2026p| 以及相关文献中，已给出静力学位错震源的震源系数，这里不再重复。以下补充对应单力源的震源系数。
...
:eq:`static_source_pot3` 式和 :eq:`static_source_u_qwv` 额外提出的虚数单位是为了后续方便定义方向因子。将 :eq:`static_source_u_pot3` 式和 :eq:`static_source_pot3` 式代入 :eq:`static_source_u_qwv` 式，得到系数 :math:`q_m, v_m, w_m` 和垂直波函数之间的关系，

.. math:: 
    :label: static_source_qwv_pot
...
为了求解震源系数，需将 :doc:`static_uniform` 部分的最终表达式展开成 :eq:`static_source_u_qwv` 的形式，对应的垂直波函数的系数 :math:`(B_m, D_m, F_m)` 即为震源系数。具体而言，我们可将位移同一分量的不同表达式进行对比得到震源系数。将 :eq:`static_source_qwv_pot` 式代入 :eq:`static_source_u_qwv` 式，柱坐标系下的位移三分量表达式为
...
再将 :eq:`static_source_pot_BDF` 式代入，整理得到，

.. math::
    :label: static_source_u_BDF
...
将上式代入 :eq:`static_source_vert_ur` 式，利用Bessel函数的性质 :math:`J_0^{'}(x) = -J_1(x)` ，得到径向位移的积分表达式
...
将上式与 :eq:`static_source_u_BDF` 式中的径向位移 :math:`u_r` 表达式对比，得到震源系数
...
为简化表达式和系数，将 :eq:`static_source_pot_BDF` 式代入 :eq:`static_source_u_pot3` 式，结合以上解的具体形式，提出公共因子，整理得到势函数的表达式，

.. math::
    :label: static_source_pot_A_PS
...
.. math::
    :label: static_source_smfld_r2R3
...
将 :eq:`static_source_smfld_1R` 式和 :eq:`static_source_smfld_r2R3` 式代入 :eq:`static_source_hori_ur` 式，得到
...
同样将势函数表达成 :eq:`static_source_pot_A_PS` 式，得到方向因子 [#]_
...
.. [#] 水平力源的 :math:`A_4` 符号与 |yao2026p| 中所使用的相反，对应的方向因子 :math:`SH_m` 也相反，这对最终位移计算结果无影响。
