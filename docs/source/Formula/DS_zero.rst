...
.. math::
    :label: DS_zero_coef_PSV

    P_1 &= 2 \varepsilon k \\
    SV_1 &= \dfrac{2k^2 - k_\beta^2}{b} \\

其中 :math:`\varepsilon` 为符号变量（对应 :eq:`DS_zero_full_qw` 式中震源系数的上标正负号）。
只需将 :eq:`DS_zero_coef_PSV` 式和 :eq:`DS_zero_free_R` 式代入 :eq:`DS_zero_full_qw` 式，即可证明 :eq:`DS_zero_full_qw` 式中高亮部分恒为0。
