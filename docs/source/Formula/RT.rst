...
在 |yao2026| 中给出了反射透射系数矩阵（下称 R/T 矩阵）的推导，其中推导的基础是先计算出固体-固体界面两侧的垂直波函数转换矩阵 :math:`\mathbf{Q}(j^-,j^+) = \mathbf{D}_{j-1}^{-1} \mathbf{D}_{j}` ，但这种推导不太适用于后续对于液体-固体界面的 R/T 矩阵的推导，故这里给出另一种推导方式，本质还是矩阵的各种变换。
