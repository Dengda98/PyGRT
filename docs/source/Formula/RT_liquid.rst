...
此时下层没有向上传播的入射波，即 :math:`[\phi_m^- (z_1^+), \psi_m^- (z_1^+)]^T = \mathbf{0}` ，:eq:`RT_liquid_layer12` 式变为
...
此时上层没有向下传播的入射波，即 :math:`\phi_m^+ (z_1^-) = 0` ，:eq:`RT_liquid_layer12` 式变为
...
其中高亮部分的垂直波函数同样为当前情况的“已知项”，为保持与 :eq:`RT_liquid_U2D` 式的形式匹配，通过类似的移项+矩阵重排的方式可得到
...
矩阵中的负号由移项产生，等号左边形式与 :eq:`RT_liquid_U2D` 式完全一致。同样该适定方程可简单使用逆矩阵求解，得到
...
:eq:`RT_liquid_U2D` 式和 :eq:`RT_liquid_D2U` 式可合并，一并使用逆矩阵求得最终液体-固体界面上的 R/T 矩阵，
