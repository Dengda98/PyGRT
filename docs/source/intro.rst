...
**PyGRT** 是一个面向层状介质理论模拟的 **C/Python** 程序包。
**GRT** 指广义反射透射系数矩阵法
(**G**\eneralized **R**\eflection-**T**\ransmission coefficient matrix)，
是程序包的核心理论基础之一； **grt** 也是程序包中由 C 语言构建的可执行文件名。

目前， **PyGRT** 可实现：

+ **动态全波解**：格林函数、合成位移、应力/应变/旋转张量等
+ **动态面波解**：频散曲线、本征函数、面波合成等
+ **静态解**：静态格林函数、合成位移、库仑应力等
+ **辅助模块**：核函数、Lamb 问题、Okada 解、格式转换等
...
**PyGRT** 运行平台
---------------------------------------
**PyGRT** 发布了二进制安装包，支持在 **Windows, Linux, MacOS** 中直接下载使用，无需编译。详见 :doc:`install` 。
