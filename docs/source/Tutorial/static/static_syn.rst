...
程序也支持在合成阶段指定新的 XY 网格。
对于每个新网格节点，程序先使用格林函数库中包围该节点震中距的采样点分别完成合成，
再根据该节点的震中距计算权重并组合这些合成结果；
如果还需要根据深度进行权重组合，则先对包围目标深度的各个深度组合分别完成上述合成，
再根据目标深度计算权重并组合结果。这里组合的是已经完成震源合成的结果，而不是直接对格林函数数组插值。
只要新的网格中的震中距和深度落在格林函数库范围内，静态格林函数计算好后，合成阶段就可以复用该格林函数库，无需重新计算。
