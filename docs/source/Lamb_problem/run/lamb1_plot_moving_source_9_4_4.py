# BEGIN LAMB1 MOVING SOURCE 9.4.4
...
def solve_station(x1, x2):
    ...
    # lamb1 返回 pi^2*mu*r*u，图 9.4.4 使用 pi^2*mu*u
    return pygrt.utils.lamb1(nu=nu, tbar=tbar, azimuth=azimuth, cbar=cbar) / distance


def plot_line(ax, x1s, x2, line_index, component):
    offsets = np.arange(len(x1s)-1, -1, -1, dtype=float)
    scale = display_scales[component]
    for i, (x1, offset) in enumerate(zip(x1s, offsets), start=1):
...
x1s = np.arange(-5.0, 6.0, 2.0)
for component, name in enumerate((r"$\bar{u}_1$", r"$\bar{u}_2$", r"$\bar{u}_3$")):
    plot_line(axes[0, component], x1s, -1.5, 1, component)
    plot_line(axes[1, component], x1s, 1.5, 2, component)
...
fig.savefig("lamb1_moving_source_9_4_4.svg", bbox_inches="tight")
# END LAMB1 MOVING SOURCE 9.4.4
