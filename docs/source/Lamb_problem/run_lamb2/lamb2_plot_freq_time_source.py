"""比较 lamb2 源点导数和 greenfn 的 EX、DD、DS、SS 分量"""
...
GREEN_ROOT = Path("GRN")
OUTPUT_PATH = Path("lamb2_compare_freq_time_source.svg")
...
def calculate_lamb2() -> tuple[np.ndarray, np.ndarray]:
    """计算 lamb2 源点导数，并返回 tbar 和 ZNE 分量"""
    tbar = np.arange(NT, dtype=float) * DT * VS / STRAIGHT_DISTANCE
    _, dG_source, _, _ = pygrt.utils.lamb2(
        nu=NU,
        tbar=tbar,
...
def calculate_greenfn() -> tuple[Stream, float]:
    ...
    # greenfn 输出的是脉冲型格林函数，积分一次后与 lamb2 的阶跃型结果对应
...
def plot_comparison(tbar: np.ndarray, lamb2_data: np.ndarray, st: Stream, scale: float) -> None:
    """绘制 5×4 的源点导数对比图"""
    ...
    for row, (left_item, right_item) in enumerate(plot_rows):
        for group, (source, component) in enumerate((left_item, right_item)):
            ...
            lamb2_axis = axes[row, 2 * group]
            comparison_axis = axes[row, 2 * group + 1]
            trace = select_trace(st, source, component)
            greenfn_data = trace.data * scale
            lamb2_series = lamb2_data[:, source_index, component_index]
            y_limits = determine_y_limits(lamb2_series[plot_mask], greenfn_data[plot_mask])

            lamb2_axis.plot(tbar, lamb2_series, color="0.5", linewidth=1.2)
            comparison_axis.plot(tbar, lamb2_series, color="0.5", linewidth=1.2)
            comparison_axis.plot(tbar, greenfn_data, color="blue", linewidth=0.7)
            for ax in (lamb2_axis, comparison_axis):
...
def main() -> None:
    ...
    try:
        tbar, lamb2_data = calculate_lamb2()
        st, scale = calculate_greenfn()
        plot_comparison(tbar, lamb2_data, st, scale)
    finally:
        remove_calculation_results()
