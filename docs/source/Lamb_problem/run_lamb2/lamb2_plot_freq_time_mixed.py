"""比较 lamb2 二阶混合导数和 greenfn 的 z/r 前缀分量"""
...
GREEN_ROOT = Path("GRN")
OUTPUT_PATHS = {
    "z": Path("lamb2_compare_freq_time_mixed_z.svg"),
    "r": Path("lamb2_compare_freq_time_mixed_r.svg"),
}
...
def convert_mixed_derivatives(dG_mixed: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """将 lamb2 的混合导数转为 greenfn 的 z/r EX、DD、DS、SS 分量
...
def calculate_lamb2() -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """计算 lamb2 混合导数，并返回 tbar 和 ZNE 分量"""
    tbar = np.arange(NT, dtype=float) * DT * VS / STRAIGHT_DISTANCE
    _, _, _, dG_mixed = pygrt.utils.lamb2(
        nu=NU,
        tbar=tbar,
    ...
    for row, (left_item, right_item) in enumerate(plot_rows):
        for group, (source, component) in enumerate((left_item, right_item)):
            ...
            for ax in (lamb2_axis, comparison_axis):
                ax.plot(tbar, lamb2_series, color="0.5", linewidth=1.2)
...
def main() -> None:
    ...
    try:
        tbar, z_data, r_data = calculate_lamb2()
