...
C 和 Python 导出的核函数文件是一致的，底层调用的是相同的函数。文件名称格式为 ``K_{iw}_{freq}``，其中 ``{iw}`` 表示频率索引值， ``{freq}`` 表示对应频率(Hz)。文件为自定义的二进制文件， **强烈建议使用Python进行读取及后续处理**。这里还是给出两种读取方法。
...
假设其它参数不变，我们调整 **震源深度为0.0km**（与场点同深），再计算格林函数，读取核函数文件，绘制图像。
